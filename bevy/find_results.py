...
class Result[T]:
    """Bevy's result types allow values to be fetched from a container."""
    ...
    def _call_factory(self, factory: t.Callable) -> t.Any:
        """
        Call a factory function and return the result.

        Supports:
        - Factories with no parameters: lambda: SomeType()
        ...
        if len(factory_sig.parameters) > 0:
            ...
            return self.container.call(factory)
        else:
    ...
    def _create_instance(self, dependency: t.Type, context: dict[str, Any]) -> tuple[Any, bool]:
        ...
        match self.container.registry.hooks[Hook.CREATE_INSTANCE].handle(self.container, dependency, context):
        ...
        filtered_instance = self.container.registry.hooks[Hook.CREATED_INSTANCE].filter(self.container, instance, context)
        return filtered_instance, disable_implicit_caching

    def _handle_unsupported_dependency(self, dependency: t.Type, context: dict[str, Any]) -> Any:
        """Handle a dependency that has no factory or existing instance.

        Delegates to hooks or raises DependencyResolutionError.
        """
        match self.container.registry.hooks[Hook.HANDLE_UNSUPPORTED_DEPENDENCY].handle(self.container, dependency, context):
            case Optional.Some(v):
                return v
    ...
    def get(self) -> T:
        """Fetches the value from the container synchronously."""
        disable_implicit_caching = False
        ...
        if qualifier:
            ...
            if self.container.parent:
                try:
                    ...
                    return self.container.parent.find(self.dependency, **parent_kwargs).get()
            ...
            if default_factory:
                ...
                # Call factory
                instance = self._call_factory(default_factory)
        ...
        # No default factory, use normal resolution with hooks
        match self.container.registry.hooks[Hook.GET_INSTANCE].handle(self.container, self.dependency, context):
            case Optional.Some(v):
                instance = v
                disable_implicit_caching = True  # Hook should handle caching
            ...
            case Optional.Nothing():
                if dep := self._get_existing_instance(self.dependency):
                    ...
                    if self.container.parent:
                        ...
                        dep = self.container.parent.find(self.dependency, default=None).get()
                    ...
                    if dep is None:
                        if "default" in self.kwargs:
                            ...
                            dep, disable_implicit_caching = self._create_instance(self.dependency, context)
        ...
        instance = self.container.registry.hooks[Hook.GOT_INSTANCE].filter(self.container, instance, context)
