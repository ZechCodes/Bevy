...
A dependency class object can implement the `bevy.injector.InjectorProtocol` by defining a `__bevy_inject__` classmethod. The context manager will call this method when it is injecting dependencies into a new instance of an injectable class.  The `__bevy_inject__` method will be passed the context, the partial instance being built, and all arguments passed to the constructor. It should return an instance of the injector dependency class.
```py
class MyInjector:
    @classmethod
    def __bevy_inject__(cls, context: bevy.Context, instance, *args, **kwargs):
        return cls()
    ...
```
The return from `__bevy_inject__` will not be added to the context's repository. If the instance being return needs to have its own dependencies injected, it will be necessary to use `context.create(MyInject, args, here)` and return that.
